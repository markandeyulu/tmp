...
public class RequirementDAOImpl implements RequirementDAO {
	...
	private ArrayList<Requirement> populateTable(ResultSet rs) throws SQLException {
		...
		while(rs.next()){
			...
			/*int custShortlt = Integer.parseInt(rq.getCustomershortlisted());
			int intEval=((rq.getProfileshared()) - (custShortlt));
			rq.setInternalEvaluation1(intEval);*/
	...
	public ArrayList<Requirement> getRequirementTable(String userId){
		StringBuffer sql = new StringBuffer("SELECT R.ID, C1.VALUE, C2.VALUE, C3.VALUE, INTIMATION_DATE,EXPECTED_DOJ, C4.VALUE, C6.VALUE, COUNT(DISTINCT R1.REQUIREMENT_ID), \r\n")
        .append("R.SHORTLISTED_PROFILE_ID, COUNT(DISTINCT R1.INTERNAL_EVALUATION_RESULT) FROM REQUIREMENT R \r\n ") 
	...
	public ArrayList<Requirement> getRequirementTable(Requirement requirement, String userId){
		...
		try {
			...
			StringBuilder queryString = new StringBuilder("SELECT R.ID, C1.VALUE, C2.VALUE, C3.VALUE, INTIMATION_DATE,EXPECTED_DOJ, "
					+ "C4.VALUE, C6.VALUE, COUNT(DISTINCT R1.REQUIREMENT_ID), R.SHORTLISTED_PROFILE_ID, COUNT(DISTINCT R1.INTERNAL_EVALUATION_RESULT) FROM REQUIREMENT R "
