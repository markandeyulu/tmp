...
var g_sourceOfProfileArray = [];
	...
	$.each(sourceOfProfileData, function(key,value) {
	    var g_item = [];
	        g_item.push(key);
	        g_item.push(value);
	 ...
	 $("#profileSourceProfile,#profileSource").append('<option value="">Select Profile Source</option>');                    	                   	             	
	     $.each(g_sourceOfProfileArray, function(i) {
	         var g_sourceOfProfileItem = g_sourceOfProfileArray[i];
	             $("#profileSourceProfile,#profileSource").append('<option id="' + g_sourceOfProfileItem[0] + '" value="' + g_sourceOfProfileItem[0] + '">' + g_sourceOfProfileItem[1] + '</option>');
	            });	
...
var g_customerEvaluationResultArray = [];	
	 $.each(customerEvaluationResultData, function(key,value) {
	    var g_item = [];
	        g_item.push(key);
	        g_item.push(value);	             		
	       ...
	       $("#customerInterviewStatusProfile,#customerInterviewStatusAdd").append('<option id="' + g_customerEvaluationResultItem[0] + '" value="' + g_customerEvaluationResultItem[0] + '">' + g_customerEvaluationResultItem[1] + '</option>');
	       });	
	       
var initialEvaluationResultData=initialEvaluationResultJson.locationJson;   
var g_initialEvaluationResultArray = [];	
$.each(initialEvaluationResultData, function(key,value) {
   var g_item = [];
       g_item.push(key);
       g_item.push(value);	             		
       g_initialEvaluationResultArray.push(g_item);	             		
      ...
      $.each(g_initialEvaluationResultArray, function(i) {
      var g_initialEvaluationResultItem = g_initialEvaluationResultArray[i];
      $("#initialEvaluationResultProfile,#initialEvaluationResultAdd").append('<option id="' + g_initialEvaluationResultItem[0] + '" value="' + g_initialEvaluationResultItem[0] + '">' + g_initialEvaluationResultItem[1] + '</option>');
      });	
