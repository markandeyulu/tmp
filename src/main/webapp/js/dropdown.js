$(document).ready(function(){
	...
	var primarySkillData=primarySkillJson.locationJson;
    	                ...
    	                var g_primarySkillArray = [];	
                          $.each(primarySkillData, function(key,value) {
    		              var g_item = [];
    		               g_item.push(key);
    		               g_item.push(value);	             		
    	...
    	$.each(g_primarySkillArray, function(i) {
    		...
    		$("#primarySkillNew,#primarySkill1").append('<option id="' + g_primarySkillItem[0] + '" value="' + g_primarySkillItem[0] + '">' + g_primarySkillItem[1] + '</option>');
    	});	
...
/*    	var projectData=[
    	...
    	var projectData = projectValues;
	             	...
	             	$.each(criticalityData, function(key,value) {
	             		var g_item = [];
	             		g_item.push(key);
	             		g_item.push(value);         		
	             	...
	             	$.each(g_criticalityArray, function(i) {
	             		...
	             		$("#criticalityNew,#criticality1").append('<option id="' + g_criticalityItem[0] + '" value="' + g_criticalityItem[0] + '">' + g_criticalityItem[1] + '</option>');
	             	});
...
var requirementTypeData=requirementTypeJson.locationJson;
	                ...
	                var g_requirementTypeArray = [];	
                      $.each(requirementTypeData, function(key,value) {
		              var g_item = [];
		               g_item.push(key);
		               g_item.push(value);	             		
	...
	$.each(g_requirementTypeArray, function(i) {
		...
		$("#requirementTypeNew,#requirementTypeAdd").append('<option id="' + g_requirementTypeItem[0] + '" value="' + g_requirementTypeItem[0] + '">' + g_requirementTypeItem[1] + '</option>');
	});
...
if(projectListData==undefined || projectListData==""){
	...
	var skillCategoryData=skillCategoryJson.locationJson;
	    	                ...
	    	                var g_skillCategoryArray = [];	
	                          $.each(skillCategoryData, function(key,value) {
	    		              var g_item = [];
	    		               g_item.push(key);
	    		               g_item.push(value);	             		
	    	...
	    	$.each(g_skillCategoryArray, function(i) {
	    		...
	    		$("#skillCategoryNew,#skillCategoryAdd1").append('<option id="' + g_skillCategoryItem[0] + '" value="' + g_skillCategoryItem[0] + '">' + g_skillCategoryItem[1] + '</option>');
	    	});		 	    	    		    	    	             	
var intimatorModeData=intimationModeJson.locationJson
	 	    	    		    	    	             		
var g_intimatorModeArray = [];
	$.each(intimatorModeData, function(key,value) {
	var g_item = [];
	g_item.push(key);
	g_item.push(value);
...
$("#intimationModeNew,#intimationModeAdd").append('<option id="' + g_intimatorModeItem[0] + '" value="' + g_intimatorModeItem[0] + '">' + g_intimatorModeItem[1] + '</option>');
});  
...
$.each(g_statusArray, function(i) {
	 ...
	 $("#statusNew").append('<option id="' + g_statusItem[0] + '" value="' + g_statusItem[0] + '">' + g_statusItem[1] + '</option>');
 });   
	 	    	    		    	    	              	  
var oppurtunityStatusData=opportunityStatusJson.locationJson;
var g_oppurtunityStatusArray = [];
$.each(oppurtunityStatusData, function(key,value) {
	 var g_item = [];
	 	 g_item.push(key);
	 	 g_item.push(value);
 ...
 $("#oppurtunityNew").append('<option value="">Select Opportunity Status</option>');                	             	
	 ...
	 $("#oppurtunityNew").append('<option id="' + g_oppurtunityStatusItem[0] + '" value="' + g_oppurtunityStatusItem[0] + '">' + g_oppurtunityStatusItem[1] + '</option>');
 });                	  
